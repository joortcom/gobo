...
feature {ANY}
   ...
   print_student_addr_direct_field(u: RESEARCH_ASSISTANT) is
      do io.put_string(u.name + " as STUDENT.addr: " + u.student_addr + "%N") end
   print_faculty_addr_direct_field(u: RESEARCH_ASSISTANT) is
      do io.put_string(u.name + " as FACULTY.addr: " + u.faculty_addr + "%N") end

   -- correct implementation: use semantic assigning accessor
   print_student_addr_via_accessor(u: RESEARCH_ASSISTANT) is
      do io.put_string(u.name + " as STUDENT.addr: " + u.get_student_addr() + "%N") end
   print_faculty_addr_via_accessor(u: RESEARCH_ASSISTANT) is
